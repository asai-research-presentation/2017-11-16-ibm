...
Note to Basel people: this presentation is aimed at the general audience who may
not be familiar or even haven't heard about AI planning.
#+end_center
...
+ Not handling "Natural Language":
  ...
  #+begin_smaller
  Pick up that .. ur .. parcel over there ... yes, it has a label on it, it says Parcel1, you can see
  it from here, the Location B. Then put it in the car, I mean the truck, the red one whose rear bumper is a bit rusty.
  #+end_smaller
#+end_span6
#+end_row-fluid
#+end_container-fluid
...
+ Semi-MDP contains Action Symbols :: =move= and =interact= (Playroom)
...
They are *randomly sampled* transitions
...
+ *Invalid transitions /never happen/ →  
  an autonomous agent /cannot possibly collect data/ in the real world.*
...
+ *Latplan:* *The /first system/ in history* which completely automatically
  generates a classical, symbolic planning model from scratch
